...
int nbrRefreshItem = 0, refreshOnOff = 0;
...
int refresh_thread(SceSize args, void *argp)  {
  ...
  nbrRefreshItem = refresh_data.refreshed;
  sprintf(dialog, "VitaConnect: Refreshed %d items :)", nbrRefreshItem);
  
EXIT:
  refreshOnOff = 0;
  utf8_to_utf16((uint8_t *)"ok", params.buttonRightText);
  utf8_to_utf16((uint8_t *)dialog, params.dialogText);
...
int license_thread(SceSize args, void *argp)
{
  ...
  if (parse_dir_with_callback(SCE_S_IFDIR, "ux0:license/addcont", license_dir_callback, &license_data) < 0)
	...
	nbrRefreshItem = license_data.count;
