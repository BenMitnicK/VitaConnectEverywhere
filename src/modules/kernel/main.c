...
static char uma0_blkdev[64], uma0_blkdev2[64];

static SceIoDevice uma0_dev = { "uma0:", "exfatuma0", uma0_blkdev, uma0_blkdev2, MOUNT_POINT_ID2 };
...
int shellKernelIsUma0Redirected(const char *blkdev, const char *blkdev2) {
  char k_blkdev[64], k_blkdev2[64];
...
int shellKernelRedirectUma0(const char *blkdev, const char *blkdev2) {
  uint32_t state;
  ENTER_SYSCALL(state);
  ...
  ksceKernelStrncpyUserToKernel(uma0_blkdev, (uintptr_t)blkdev, sizeof(uma0_blkdev)-1);
  ksceKernelStrncpyUserToKernel(uma0_blkdev2, (uintptr_t)blkdev2, sizeof(uma0_blkdev2)-1);
...
int module_start(SceSize args, void *argp) {
  ...
  // Load SceUsbMass
  SceUID modid = ksceKernelLoadStartModule("ur0:QuickMenuReborn/VitaConnectEverywhere/umass.skprx", 0, NULL, 0, NULL, NULL);
